...
# Added pugixml, scons, libexiv2-dev, libwayland-dev libwayland-egl1 libtinyxml2-dev, libdrm-dev
# Replaced libcdio-dev with libcdio++-dev
# Add libdisplay-info-dev libinput-dev
RUN apt install -y autoconf automake autopoint autotools-dev \
    ...
    libcurl4-openssl-dev libcwiid-dev libdbus-1-dev libdisplay-info-dev libdrm-dev libegl1-mesa-dev \
    ...
    libvorbis-dev libwayland-dev libwayland-egl1 libxkbcommon-dev \
...
RUN cd dependencies && \
	...
	make -j`nproc` -C target/dav1d PREFIX=/usr/local && \
	make -j`nproc` -C target/ffmpeg PREFIX=/usr/local PLATFORM=linux ARCHIVE_TOOL=tar CMAKE=cmake && \
